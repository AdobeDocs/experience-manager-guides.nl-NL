...
![ Instellingen voor voorinstellingen voor sites](images/aem-sites-new-config.png)
**Algemeen**
...
1. Klik **toevoegen**. The preset for AEM Sites is created.
1. You can configure the Out-of-the-box Sites template  in two ways:
   1. Select **Site** and then choose the publish path and the topic page templates from the populated options:
      1. Select the Site.
      1. Select **Site**. Bijvoorbeeld `AEMG Docs` .
   ...
   1. Select the complete Site path:
      ...
      1. Select the complete Site path. Bijvoorbeeld `/content/AEMG-Docs-Site/en/docs/product1` .
...
1. Generate AEM Sites for the corresponding map. Bijvoorbeeld `/content/AEMG-Docs-Site/en/docs/product` .
...
1. Open the **Cross map references** tab.
   ...
   To use the cross-map reference panel from Web Editor, `<xrefs>` must have unique IDs. Unieke id&#39;s voor `<xrefs>` worden automatisch gegenereerd bij het bewerken/opslaan van de oudere inhoud als de id er niet is.
