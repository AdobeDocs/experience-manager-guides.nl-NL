---
title: Aanbevelingen voor optimalisatie van prestaties voor Op locatie
...
# Aanbevelingen voor optimalisatie van prestaties voor Op locatie {#id213BD0JG0XA}
...
**wat is de verandering?**
Uitsluiten `/var/dxml from oak:index/lucene` .

>[!NOTE]
>
> AEM Guides gebruikt nooit Lucene-indexen om te zoeken naar inhoud in knooppunt `/var/dxml` .

**Wanneer te vormen?**
Als u deze wijziging aanbrengt op een nieuw systeem voordat u inhoud gaat migreren, hoeft u `oak:index/lucene` alleen bij te werken. Anders, op een bestaand systeem waar de inhoud reeds wordt gemigreerd, dan na het aanbrengen van de verandering in `oak:index/lucene,` herbouwt indexen voor Luzern \ (*die enkele uren zou kunnen vergen om* \) te voltooien.

**Resultaat van deze verandering**
Door deze wijziging wordt voorkomen dat het knooppunt `/var/dxml` wordt geïndexeerd en opgeslagen in de segmentopslag.
