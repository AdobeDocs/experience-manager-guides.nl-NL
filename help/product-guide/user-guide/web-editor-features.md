...
- **New**: Create a new DITA topic, DITA map, or a folder.
  ...
  Steps to create a new topic:
   ...
   3. In the **New topic** dialog box, provide the following details:
      ...
      - \(Optional\)* The file name for the topic. De bestandsnaam wordt automatisch voorgesteld op basis van de titel van het onderwerp. In case your administrator has enabled automatic file names based on UUID setting, then you will not view the Name field.
      - Een malplaatje waarop het onderwerp zal worden gebaseerd. For example, for an out-of-the-box setup, you can choose from the Blank, Concept, DITAVAL, Reference, Task, Topic, Markdown, Glossary, and Troubleshooting templates. If your folder has a Folder Profile configured on it, then you will view only those topic templates that are configured on the Folder profile.
   ...
   4. Selecteer **creeer**. The topic is created at the specified path. Also, the topic is opened in the Editor for editing.
  ...
  Steps to create a new DITA map:
   ...
   3. In the **New map** dialog box, provide the following details:
      - A Title for the map.
      - *\(Optioneel\)* De bestandsnaam voor de kaart. The file name is auto-suggested based on the map title. In case your administrator has enabled automatic file names based on UUID setting, then you will not view the Name field.
   ...
   4. Selecteer **creeer**. The map is created and added within the folder specified in the Path field. Also, the map is opened in the Map view. U kunt het kaartdossier in de Redacteur van de Kaart openen en onderwerp aan het toevoegen. For more information about adding topics to a map file, view [Create a map](map-editor-create-map.md#). Alternatively, select **Open in map console** to open the map in Map console.
  ...
  Steps to create a new folder:
   ...
   3. In the **New folder** dialog box, provide the following details:
      ...
      - Path where you want to save the folder. Standaard wordt het pad van de geselecteerde map in de opslagplaats weergegeven in het veld Pad.
   4. Selecteer **creeer**. De map wordt gemaakt en toegevoegd in de map vanwaar de optie Map maken is uitgevoerd.
...
Als u bewerkingsrechten hebt voor de kaartbestanden, kunt u de bestanden ook bewerken. For more information about opening and editing a topic through DITA map, view [Edit topics through DITA map](map-editor-advanced-map-editor.md#id17ACJ0F0FHS).
...
- **Unlock**: Unlocks the map file and makes it available for editing. De wijzigingen worden niet teruggezet naar de vorige versie.
...
- **voeg aan** toe: U kunt van de volgende opties kiezen:
   - **Collections**: Adds the map file to collections. U kunt desgewenst toevoegen aan een bestaande of nieuwe verzameling.
