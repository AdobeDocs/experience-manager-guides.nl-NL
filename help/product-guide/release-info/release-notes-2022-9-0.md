...
description: Release van Adobe Experience Manager Guides as a Cloud Service in september
...
# Release van Adobe Experience Manager Guides as a Cloud Service in september
...
1. Werk de eigenschap `<dox.version>` in het `/dox/dox.installer/pom.xml` -bestand van de Git-code voor Cloud Services bij naar 202.9.178.
1. Leg de wijzigingen vast en voer de Cloud Services-pijplijn uit om naar de release van september van AEM Guides as a Cloud Service te upgraden.
...
* Voer een POST-aanvraag uit op de server (met correcte verificatie) - `http://<server:port>/bin/guides/map-find/indexin` .
...
Deze sectie bevat een overzicht van de compatibiliteitsmatrix voor de softwaretoepassingen die worden ondersteund door de AEM Guides as a Cloud Service-release van september 2022.
...
AEM Guides as a Cloud Service biedt een groot aantal verbeteringen en nieuwe functies in de release van september:
...
* PDF-sjabloon | Als u achtergrondafbeeldingen toevoegt aan een paginalay-out, wordt het afbeeldingspad absoluut weergegeven en worden de afbeeldingen niet weergegeven in de PDF-uitvoer voor uitvoer. 10297
* Oorspronkelijke PDF | Hoofdstuktitel en hoofdstukkop werken niet in PDF-publicaties. (9947)
* Oorspronkelijke PDF | `xref` voor een concept wordt niet correct opgelost voor een specifiek onderwerp DITA. (1029)
* Oorspronkelijke PDF | Kan de bijschrifttekst voor een tabel niet weergeven in de gegenereerde PDF-uitvoer. 9827
* Oorspronkelijke PDF | Verwijzingen in bijlagen worden niet als bijlagen weergegeven in PDF-uitvoer. (10182)
* Oorspronkelijke PDF | Framekenmerk voor een tabel wordt niet doorgegeven aan de tijdelijke HTML (als klasse). 10353
* Oorspronkelijke PDF | temp HTML-bestanden voegen de klassen colsep en rowsep toe aan td en de klassen zelfs als de waarde 0 is in de bron-DITA. 10352
* Oorspronkelijke PDF |  Metagegevens voor criteria die in paginalay-out zijn toegevoegd, worden niet ondersteund. 10377
* Oorspronkelijke PDF |  Genereren van PDF mislukt voor specifieke inhoud. (9927)
* Oorspronkelijke PDF | Inhoud via conkeyref wordt niet weergegeven in de PDF-uitvoer. 9836
* Oorspronkelijke PDF | Belangrijke referenties voor Keydefs met afbeeldingen of externe koppelingen zijn niet opgelost. 10063
...
* Met Control + F wordt de zoekmodus van de browser niet geopend in de Assets-console nadat AEM Guides 4.1 op AEM 6.5.12 is geïnstalleerd. 10189


## Bekende problemen

Adobe heeft de volgende bekende problemen voor de release van AEM Guides as a Cloud Service september 2022 geïdentificeerd.
